...
async def main(filename: str , filename_out: str):
    print(f"Opening image: {filename}")
    with open(filename, "rb") as f:
        raw_bytes = f.read()
    print("Processing image...")
    processed_image = await process_image(raw_bytes, rembg_session)
...
if __name__ == "__main__":
    ...
    asyncio.run(main(filename=args.input, filename_out=args.output))
