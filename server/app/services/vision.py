...
from rembg.bg import ReturnType
...
def process_image(raw_bytes: bytes, rembg_session: BaseSession) -> np.ndarray:
    """
    ...
    img_arr = cv2.cvtColor(np.array(pil_img), cv2.COLOR_RGBA2BGRA)  # RGBA → BGRA for OpenCV
	...
	final_img = flatten_to_color(processed_img)  # optional: composite onto white background for wrinkle smoothing
    _, png_arr = cv2.imencode('.png', final_img)
    return png_arr
...
def pad_to_square(img: np.ndarray, size:int = 768, padding: float = 0.08) -> np.ndarray:
	...
	"""
	# pad_color is BGRA — default is white padding
	h, w = img.shape[:2]
